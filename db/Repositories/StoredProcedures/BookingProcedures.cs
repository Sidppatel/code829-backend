...
{
    public async Task<Guid> CreateBookingAsync(Guid userId, Guid eventId, Guid? tableId, int? seats, int subtotalCents, int feeCents, int totalCents, string bookingNumber, string status = "Pending", CancellationToken ct = default)
    {
        var result = await context.Database
            .SqlQueryRaw<Guid>(
                "SELECT sp_create_booking(@p0, @p1, @p2, @p3, @p4, @p5, @p6, @p7, @p8) AS \"Value\"",
