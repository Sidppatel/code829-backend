...
BEGIN
    ...
    IF v_id IS NULL THEN
        INSERT INTO users (""Id"", ""Email"", ""EmailHash"", ""FirstName"", ""LastName"", ""Role"", ""IsActive"", ""LastLoginAt"", ""OptInLocationEmail"", ""HasCompletedOnboarding"", ""CreatedAt"", ""UpdatedAt"")
        VALUES (gen_random_uuid(), p_email, p_email_hash, p_first_name, p_last_name, p_role, true, now(), false, false, now(), now())
        RETURNING ""Id"" INTO v_id;
    ELSE
...
CREATE OR REPLACE FUNCTION sp_create_device_session(
    p_user_id uuid, p_session_hash text, p_fingerprint text, p_device_name text, p_ip text, p_expires_at timestamptz
) RETURNS uuid LANGUAGE plpgsql AS $$
DECLARE v_id uuid;
BEGIN
    INSERT INTO device_sessions (""Id"", ""UserId"", ""SessionHash"", ""DeviceFingerprint"", ""DeviceName"", ""IpAddress"", ""LastActivityAt"", ""ExpiresAt"", ""CreatedAt"", ""UpdatedAt"")
    VALUES (gen_random_uuid(), p_user_id, p_session_hash, p_fingerprint, p_device_name, p_ip, now(), p_expires_at, now(), now())
    RETURNING ""Id"" INTO v_id;
    RETURN v_id;
END; $$;
");
...
BEGIN
    UPDATE device_sessions SET ""RevokedAt"" = now(), ""UpdatedAt"" = now() WHERE ""SessionHash"" = p_session_hash AND ""RevokedAt"" IS NULL;
END; $$;
");

            migrationBuilder.Sql(@"
CREATE OR REPLACE FUNCTION sp_revoke_all_user_sessions(p_user_id uuid, p_except_hash text DEFAULT NULL) RETURNS int LANGUAGE plpgsql AS $$
DECLARE v_count int;
BEGIN
    UPDATE device_sessions SET ""RevokedAt"" = now(), ""UpdatedAt"" = now()
    WHERE ""UserId"" = p_user_id AND ""RevokedAt"" IS NULL AND (p_except_hash IS NULL OR ""SessionHash"" <> p_except_hash);
...
DECLARE v_count int;
BEGIN
    DELETE FROM device_sessions WHERE ""ExpiresAt"" < now() OR ""RevokedAt"" IS NOT NULL AND ""RevokedAt"" < now() - interval '7 days';
    GET DIAGNOSTICS v_count = ROW_COUNT;
    RETURN v_count;
END; $$;
");
...
BEGIN
    ...
    IF v_address_id IS NULL AND (p_address IS NOT NULL OR p_city IS NOT NULL) THEN
        ...
        VALUES (gen_random_uuid(), COALESCE(p_address,''), COALESCE(p_city,''), COALESCE(p_state,''), COALESCE(p_zip,''), now(), now())
...
");
            ...
            migrationBuilder.Sql(@"
CREATE OR REPLACE FUNCTION sp_update_user_avatar(p_user_id uuid, p_avatar_path text) RETURNS void LANGUAGE plpgsql AS $$
BEGIN
...
");
            ...
            migrationBuilder.Sql(@"
CREATE OR REPLACE FUNCTION sp_update_user_role(p_user_id uuid, p_role text) RETURNS void LANGUAGE plpgsql AS $$
BEGIN
...
BEGIN
    UPDATE events SET
        ...
        ""PricePerPersonCents"" = p_price_per_person_cents,
        ""PlatformFeePercent"" = p_platform_fee_percent,
...
");
            ...
            migrationBuilder.Sql(@"
CREATE OR REPLACE FUNCTION sp_change_event_status(p_id uuid, p_status text, p_scheduled_publish_at timestamptz DEFAULT NULL) RETURNS void LANGUAGE plpgsql AS $$
BEGIN
    UPDATE events SET
        ""Status"" = p_status,
...
DECLARE v_count int;
BEGIN
    UPDATE events SET ""Status"" = 'Published', ""PublishedAt"" = now(), ""ScheduledPublishAt"" = NULL, ""UpdatedAt"" = now()
    WHERE ""Status"" = 'Draft' AND ""ScheduledPublishAt"" IS NOT NULL AND ""ScheduledPublishAt"" <= now();
    GET DIAGNOSTICS v_count = ROW_COUNT;
    RETURN v_count;
END; $$;
");
...
CREATE OR REPLACE FUNCTION sp_create_venue(
    p_name text, p_description text, p_image_path text, p_phone text, p_email text, p_website text,
...
BEGIN
    ...
    VALUES (gen_random_uuid(), COALESCE(p_line1,''), p_line2, COALESCE(p_city,''), COALESCE(p_state,''), COALESCE(p_zip,''), now(), now())
    RETURNING ""Id"" INTO v_addr_id;

    INSERT INTO venues (""Id"", ""Name"", ""Description"", ""ImagePath"", ""Phone"", ""Email"", ""Website"", ""IsActive"", ""AddressId"", ""CreatedAt"", ""UpdatedAt"")
    VALUES (gen_random_uuid(), p_name, p_description, p_image_path, p_phone, p_email, p_website, true, v_addr_id, now(), now())
    RETURNING ""Id"" INTO v_id;
    RETURN v_id;
END; $$;
");
...
BEGIN
    ...
    IF v_addr_id IS NOT NULL THEN
        UPDATE addresses SET ""Line1"" = COALESCE(p_line1, ""Line1""), ""City"" = COALESCE(p_city, ""City""),
            ""State"" = COALESCE(p_state, ""State""), ""ZipCode"" = COALESCE(p_zip, ""ZipCode""), ""UpdatedAt"" = now()
        WHERE ""Id"" = v_addr_id;
    END IF;
    UPDATE venues SET ""Name"" = COALESCE(p_name, ""Name""), ""Description"" = COALESCE(p_description, ""Description""),
        ""ImagePath"" = COALESCE(p_image_path, ""ImagePath""), ""Phone"" = p_phone, ""Email"" = p_email,
        ""Website"" = p_website, ""IsActive"" = COALESCE(p_is_active, ""IsActive""), ""UpdatedAt"" = now()
    WHERE ""Id"" = p_id;
END; $$;
");
...
DECLARE v_id uuid;
BEGIN
    INSERT INTO event_tables (""Id"", ""EventId"", ""Label"", ""Capacity"", ""Shape"", ""Color"", ""PriceCents"", ""PlatformFeeCents"", ""IsActive"", ""TableTemplateId"", ""CreatedAt"", ""UpdatedAt"")
    VALUES (gen_random_uuid(), p_event_id, p_label, p_capacity, p_shape, p_color, p_price_cents, p_platform_fee_cents, true, p_template_id, now(), now())
    RETURNING ""Id"" INTO v_id;
    RETURN v_id;
END; $$;
");
...
DECLARE v_id uuid;
BEGIN
    INSERT INTO tables (""Id"", ""EventTableId"", ""EventId"", ""Label"", ""GridRow"", ""GridCol"", ""SortOrder"", ""IsActive"", ""Status"", ""CreatedAt"", ""UpdatedAt"")
    VALUES (gen_random_uuid(), p_event_table_id, p_event_id, p_label, p_grid_row, p_grid_col, p_sort_order, true, 'Available', now(), now())
    RETURNING ""Id"" INTO v_id;
    RETURN v_id;
END; $$;
");
            ...
            migrationBuilder.Sql(@"
CREATE OR REPLACE FUNCTION sp_lock_table(p_user_id uuid, p_event_id uuid, p_table_id uuid, p_hold_minutes int)
RETURNS TABLE(""Id"" uuid, ""Label"" text, ""LockExpiresAt"" timestamptz) LANGUAGE plpgsql AS $$
BEGIN
    RETURN QUERY
    UPDATE tables SET ""Status"" = 'Locked', ""LockedByUserId"" = p_user_id,
        ""LockExpiresAt"" = now() + (p_hold_minutes || ' minutes')::interval, ""UpdatedAt"" = now()
...
BEGIN
    UPDATE tables SET ""Status"" = 'Booked', ""LockedByUserId"" = NULL, ""LockExpiresAt"" = NULL, ""UpdatedAt"" = now() WHERE ""Id"" = p_table_id;
END; $$;
");
...
");
            ...
            migrationBuilder.Sql(@"
CREATE OR REPLACE FUNCTION sp_confirm_booking(p_booking_id uuid, p_qr_token text) RETURNS void LANGUAGE plpgsql AS $$
...
BEGIN
    ...
    FOR v_seat IN 1..v_seats LOOP
        INSERT INTO booking_tickets (""Id"", ""BookingId"", ""TicketCode"", ""QrToken"", ""SeatNumber"", ""Status"", ""CreatedAt"", ""UpdatedAt"")
...
BEGIN
    ...
    RETURNING ""TableId"" INTO v_table_id;
...
DECLARE v_id uuid;
BEGIN
    INSERT INTO payments (""Id"", ""BookingId"", ""PaymentIntentId"", ""Status"", ""AmountCents"", ""Currency"", ""CreatedAt"", ""UpdatedAt"")
    VALUES (gen_random_uuid(), p_booking_id, p_intent_id, 'RequiresConfirmation', p_amount_cents, p_currency, now(), now())
    RETURNING ""Id"" INTO v_id;
    RETURN v_id;
END; $$;
");
            ...
            migrationBuilder.Sql(@"
CREATE OR REPLACE FUNCTION sp_update_payment_status(p_intent_id text, p_status text) RETURNS void LANGUAGE plpgsql AS $$
BEGIN
    UPDATE payments SET
        ""Status"" = p_status,
...
CREATE OR REPLACE FUNCTION sp_create_image(
    ...
    p_size_bytes int, p_width int, p_height int, p_is_primary bool, p_sort_order int, p_uploaded_by uuid
) RETURNS uuid LANGUAGE plpgsql AS $$
DECLARE v_id uuid;
BEGIN
    ...
    INSERT INTO images (""Id"", ""EntityType"", ""EntityId"", ""StorageKey"", ""OriginalName"",
        ""SizeBytes"", ""Width"", ""Height"", ""IsPrimary"", ""SortOrder"", ""UploadedById"", ""CreatedAt"", ""UpdatedAt"")
    VALUES (gen_random_uuid(), p_entity_type, p_entity_id, p_storage_key, p_original_name,
        p_size_bytes, p_width, p_height, p_is_primary, p_sort_order, p_uploaded_by, now(), now())
    RETURNING ""Id"" INTO v_id;
    RETURN v_id;
END; $$;
");
...
");
            ...
            migrationBuilder.Sql(@"
CREATE OR REPLACE FUNCTION sp_upsert_setting(p_key text, p_encrypted_value text, p_description text DEFAULT NULL) RETURNS void LANGUAGE plpgsql AS $$
BEGIN
    ...
    ON CONFLICT (""Key"") DO UPDATE SET ""EncryptedValue"" = EXCLUDED.""EncryptedValue"",
        ""Description"" = COALESCE(EXCLUDED.""Description"", app_settings.""Description""), ""UpdatedAt"" = now();
END; $$;
");
...
CREATE OR REPLACE FUNCTION sp_create_admin_log(
    ...
    p_entity_type text, p_entity_id uuid, p_description text, p_metadata_json text, p_ip text
) RETURNS uuid LANGUAGE plpgsql AS $$
DECLARE v_id uuid;
BEGIN
...
DECLARE v_id uuid;
BEGIN
    INSERT INTO developer_logs (""Id"", ""Timestamp"", ""Severity"", ""Message"", ""ExceptionType"", ""StackTrace"",
        ""RequestPath"", ""RequestMethod"", ""StatusCode"", ""UserId"", ""IpAddress"", ""CorrelationId"", ""MetadataJson"")
    VALUES (gen_random_uuid(), now(), p_severity, p_message, p_exception_type, p_stack_trace,
        p_request_path, p_request_method, p_status_code, p_user_id, p_ip, p_correlation_id, p_metadata_json)
    RETURNING ""Id"" INTO v_id;
    RETURN v_id;
END; $$;
");
...
CREATE OR REPLACE FUNCTION sp_create_system_log(
    p_category text, p_action text, p_source text, p_entity_type text, p_entity_id uuid,
    p_before_json text, p_after_json text, p_actor_id uuid, p_correlation_id text, p_duration_ms bigint, p_metadata_json text
) RETURNS uuid LANGUAGE plpgsql AS $$
DECLARE v_id uuid;
BEGIN
    INSERT INTO system_logs (""Id"", ""Timestamp"", ""Category"", ""Action"", ""Source"", ""EntityType"", ""EntityId"",
        ""BeforeJson"", ""AfterJson"", ""ActorId"", ""CorrelationId"", ""DurationMs"", ""MetadataJson"")
    VALUES (gen_random_uuid(), now(), p_category, p_action, p_source, p_entity_type, p_entity_id,
        p_before_json, p_after_json, p_actor_id, p_correlation_id, p_duration_ms, p_metadata_json)
    RETURNING ""Id"" INTO v_id;
    RETURN v_id;
END; $$;
");
...
");
            ...
            migrationBuilder.Sql(@"
CREATE OR REPLACE FUNCTION sp_cleanup_old_logs(p_dev_days int, p_admin_days int, p_system_days int) RETURNS int LANGUAGE plpgsql AS $$
...
DECLARE v_id uuid;
BEGIN
    INSERT INTO feedbacks (""Id"", ""Name"", ""Email"", ""Type"", ""Message"", ""Rating"", ""UserId"", ""UserAgent"", ""IpAddress"", ""CreatedAt"", ""UpdatedAt"")
    VALUES (gen_random_uuid(), p_name, p_email, p_type, p_message, p_rating, p_user_id, p_user_agent, p_ip, now(), now())
    RETURNING ""Id"" INTO v_id;
    RETURN v_id;
END; $$;
");
...
BEGIN
    IF TG_OP = 'DELETE' THEN
        INSERT INTO system_logs (""Id"", ""Timestamp"", ""Category"", ""Action"", ""Source"", ""EntityType"", ""EntityId"", ""BeforeJson"", ""AfterJson"")
        VALUES (gen_random_uuid(), now(), 'EntityChange', 'Delete', TG_TABLE_NAME, TG_TABLE_NAME,
            (OLD.""Id"")::uuid, row_to_json(OLD)::text, NULL);
        RETURN OLD;
    ELSIF TG_OP = 'UPDATE' THEN
        INSERT INTO system_logs (""Id"", ""Timestamp"", ""Category"", ""Action"", ""Source"", ""EntityType"", ""EntityId"", ""BeforeJson"", ""AfterJson"")
        VALUES (gen_random_uuid(), now(), 'EntityChange', 'Update', TG_TABLE_NAME, TG_TABLE_NAME,
            (NEW.""Id"")::uuid, row_to_json(OLD)::text, row_to_json(NEW)::text);
        RETURN NEW;
    ELSIF TG_OP = 'INSERT' THEN
        INSERT INTO system_logs (""Id"", ""Timestamp"", ""Category"", ""Action"", ""Source"", ""EntityType"", ""EntityId"", ""BeforeJson"", ""AfterJson"")
        VALUES (gen_random_uuid(), now(), 'EntityChange', 'Insert', TG_TABLE_NAME, TG_TABLE_NAME,
            (NEW.""Id"")::uuid, NULL, row_to_json(NEW)::text);
        RETURN NEW;
    END IF;
    RETURN NULL;
END; $$;
");
            ...
            // Apply audit triggers to all business tables (exclude log tables to avoid recursion)
            var auditTables = new[]
            {
...
");
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            ...
            var sps = new[]
            {
                "sp_create_magic_link", "sp_consume_magic_link", "sp_upsert_user", "sp_update_user_last_login",
                "sp_create_device_session", "sp_revoke_device_session", "sp_revoke_all_user_sessions",
                "sp_cleanup_expired_sessions", "sp_update_session_activity",
                "sp_update_user_profile", "sp_update_user_avatar", "sp_update_user_role",
                "sp_create_event", "sp_update_event", "sp_change_event_status", "sp_publish_scheduled_events",
                "sp_create_venue", "sp_update_venue",
                "sp_create_event_table", "sp_create_table", "sp_lock_table", "sp_release_table_lock",
                "sp_mark_table_booked", "sp_cleanup_expired_locks",
                "sp_create_booking", "sp_confirm_booking", "sp_cancel_booking", "sp_refund_booking",
                "sp_invite_ticket", "sp_claim_ticket",
                "sp_create_payment", "sp_update_payment_status",
                "sp_create_image", "sp_delete_image",
                "sp_upsert_setting",
                "sp_create_admin_log", "sp_create_developer_log", "sp_create_system_log", "sp_create_email_log",
                "sp_cleanup_old_logs", "sp_create_feedback"
            };
            foreach (var sp in sps)
                migrationBuilder.Sql($"DROP FUNCTION IF EXISTS {sp} CASCADE;");

            // Drop views
            migrationBuilder.Sql("DROP VIEW IF EXISTS v_events, v_event_summary, v_tables, v_bookings, v_booking_tickets, v_venues, v_user_profile, v_event_tables_summary;");
